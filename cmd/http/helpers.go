...
func runDb() {
	dsn := fmt.Sprintf(
		"host=%s user=%s password=%s dbname=%s port=%s sslmode=disable TimeZone=Europe/Zagreb application_name=creatif",
...
func createDatabase() {
	...
	if !ok {
		...
		if err := storage2.Gorm().AutoMigrate(declarations.Connection{}); err != nil {
			log.Fatalln(err)
		}
		...
		*/
		...
		if err := storage2.Gorm().AutoMigrate(published.PublishedGroups{}); err != nil {
			log.Fatalln(err)
		}
