...
func completeCleanup() {
	...
	Gorm().Exec(fmt.Sprintf("TRUNCATE TABLE declarations.%s CASCADE", CONNECTIONS_TABLE))
	...
	Gorm().Exec(fmt.Sprintf("TRUNCATE TABLE published.%s CASCADE", PUBLISHED_GROUPS_TABLE))
}
