...
var _ = ginkgo.Describe("List items pagination tests", func() {
	ginkgo.It("should paginate through list variables", func() {
		...
		handler := New(NewModel(projectId, "eng", listName, "created_at", "", "desc", 10, 1, []string{"one"}, nil), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
	...
	ginkgo.It("should get an empty result from the end of the list variables listing", func() {
		...
		handler := New(NewModel(projectId, "eng", listName, "created_at", "", "desc", 10, 50, []string{"one"}, nil), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
	...
	ginkgo.It("should return empty result for group that does not exist", func() {
		...
		handler := New(NewModel(projectId, "eng", listName, "created_at", "", "desc", 10, 1, []string{"not_exists"}, nil), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
	...
	ginkgo.It("should return the exact number of items by group", func() {
		...
		handler := New(NewModel(projectId, "eng", listName, "created_at", "", "desc", 50, 1, []string{"one"}, nil), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
	...
	ginkgo.It("should return items search by name with regex", func() {
		...
		handler := New(NewModel(projectId, "eng", listName, "created_at", "1", "desc", 10, 1, []string{}, nil), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
	...
	ginkgo.It("should return items search by name with regex with groups", func() {
		...
		handler := New(NewModel(projectId, "eng", listName, "created_at", "1", "desc", 10, 1, []string{"one"}, nil), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
