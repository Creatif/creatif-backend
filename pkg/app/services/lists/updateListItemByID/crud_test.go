...
var _ = ginkgo.Describe("Declaration (UPDATE) variable tests", func() {
	ginkgo.It("should update the name of the list item variable", func() {
		...
		handler := New(NewModel(projectId, "eng", []string{"name", "behaviour"}, "name", "", "", singleItem.ID, "", "newName", "readonly", []string{}, []byte{}, v), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
	...
	ginkgo.It("should update the groups of the declaration variable", func() {
		...
		v, err := json.Marshal(m)
		gomega.Expect(err).Should(gomega.BeNil())
		handler := New(NewModel(projectId, "eng", []string{"name", "groups", "value"}, "name", "", "", singleItem.ID, "", "newName", "readonly", []string{"first", "second", "third"}, []byte{}, v), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
	...
	ginkgo.It("should update the behaviour of the declaration variable", func() {
		...
		v, err := json.Marshal(m)
		gomega.Expect(err).Should(gomega.BeNil())
		handler := New(NewModel(projectId, "eng", []string{"name", "behaviour", "groups"}, "name", "", "", singleItem.ID, "", "newName", "readonly", []string{"first", "second", "third"}, []byte{}, v), auth.NewNoopAuthentication(false), logger.NewLogBuilder())
