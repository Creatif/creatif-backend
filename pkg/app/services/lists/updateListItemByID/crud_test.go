...
var _ = ginkgo.Describe("Declaration (UPDATE) variable tests", func() {
	...
	ginkgo.It("should update the groups of the declaration variable", func() {
		...
		v, err := json.Marshal(m)
		gomega.Expect(err).Should(gomega.BeNil())
		handler := New(NewModel(projectId, []string{"name", "groups", "value"}, "name", "newName", "newName", "readonly", []string{"first", "second", "third"}, []byte{}, v))
	...
	ginkgo.It("should update the behaviour of the declaration variable", func() {
		...
		v, err := json.Marshal(m)
		gomega.Expect(err).Should(gomega.BeNil())
		handler := New(NewModel(projectId, []string{"name", "behaviour", "groups"}, "name", "newName", "newName", "readonly", []string{"first", "second", "third"}, []byte{}, v))
