...
func (c Main) Logic() (interface{}, error) {
	if c.model.Type == "map" {
		if c.model.Type == "map" {
			if err := storage.Gorm().Transaction(func(tx *gorm.DB) error {
				mapSql := fmt.Sprintf("DELETE FROM %s AS mv USING %s AS m WHERE mv.map_id = ? AND m.project_id = ?", (declarations.MapVariable{}).TableName(), (declarations.Map{}).TableName())
	...
	if err := storage.Gorm().Transaction(func(tx *gorm.DB) error {
		mapSql := fmt.Sprintf("DELETE FROM %s AS lv USING %s AS l WHERE lv.list_id = ? AND l.project_id = ?", (declarations.ListVariable{}).TableName(), (declarations.List{}).TableName())
		if res := tx.Exec(mapSql, c.model.ID, c.model.ProjectID); res.Error != nil {
			return errors.New(res.Error.Error())
		}
