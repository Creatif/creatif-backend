...
func publishFullProject(projectId string) ([]addToMap.View, publish.View) {
	...
	referenceListItem2 := testAddToList(projectId, referenceList.ID, "reference-map-2", []connections.Connection{}, sdk.Map(groups, func(idx int, value addGroups.View) string {
		return value.ID
	}))
