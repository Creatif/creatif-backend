...
func TestApi(t *testing.T) {
	...
	GinkgoRunSpecs(t, "Public API - Get many items")
}
...
func publishFullProject(projectId string) ([]string, []string, publish.View) {
	...
	for i := 0; i < 200; i++ {
		v := testAddToMap(projectId, map1.ID, fmt.Sprintf("map-%d", i), []shared.Reference{}, sdk.Map(groups, func(idx int, value addGroups.View) string {
			return value.ID
		}))

		mapIds = append(mapIds, v.ID)
	}

	for i := 0; i < 200; i++ {
		v := testAddToList(projectId, list1.ID, fmt.Sprintf("list-%d", i), []shared.Reference{}, sdk.Map(groups, func(idx int, value addGroups.View) string {
			return value.ID
		}))

		listIds = append(listIds, v.ID)
	}
