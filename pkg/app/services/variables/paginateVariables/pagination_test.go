...
var _ = ginkgo.Describe("Variable pagination tests", func() {
	ginkgo.It("should paginate through variables", func() {
		...
		handler := New(NewModel(projectId, "eng", "created_at", "", "desc", 10, 1, []string{"one"}, nil), auth.NewTestingAuthentication(false), logger.NewLogBuilder())
	...
	ginkgo.It("should get an empty result from the end of the variables listing", func() {
		...
		handler := New(NewModel(projectId, "eng", "created_at", "", "desc", 10, 50, []string{"one"}, nil), auth.NewTestingAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
	...
	ginkgo.It("should return empty result for group that does not exist", func() {
		...
		handler := New(NewModel(projectId, "eng", "created_at", "", "desc", 10, 1, []string{"not_exists"}, nil), auth.NewTestingAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
	...
	ginkgo.It("should return the exact number of items by group", func() {
		...
		handler := New(NewModel(projectId, "eng", "created_at", "", "desc", 75, 1, []string{"one"}, nil), auth.NewTestingAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
	...
	ginkgo.It("should return variables search by name without groups", func() {
		...
		handler := New(NewModel(projectId, "eng", "created_at", "1", "desc", 10, 1, []string{}, nil), auth.NewTestingAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
	...
	ginkgo.It("should return variables search by name with groups", func() {
		...
		handler := New(NewModel(projectId, "eng", "created_at", "1", "desc", 10, 1, []string{"one"}, nil), auth.NewTestingAuthentication(false), logger.NewLogBuilder())
		views, err := handler.Handle()
		testAssertErrNil(err)
